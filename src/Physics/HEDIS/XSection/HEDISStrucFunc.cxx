...
HEDISStrucFunc::HEDISStrucFunc(SF_info sfinfo)
{
  ...
  string SFname = string(gSystem->Getenv("GENIE")) + "/data/evgen/hedis/sf/" + RunOpt::Instance()->Tune()->Name();
  ...
  if ( gSystem->mkdir(SFname.c_str())==0 ) {
    ...
    LOG("HEDISStrucFunc", pINFO) << "Comparing info from Metafile and selected Tune (CommonParameters)";
    SF_info cm;
