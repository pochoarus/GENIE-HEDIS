...
{
  LOG("HEDISGenerator", pDEBUG) << "Running HEDIS PYTHIA hadronizer";
  ...
  /* udd u(->q)ub => dd + q (u valence and ub sea annihilates)*/
    ...
    while( counter<100 ) {
      ...
      if ( WT > TMath::Power(m_frag+m_rema+fMinESinglet,2) ) {
        ...
        LOG("HEDISGenerator", pINFO) << "Not backward hadron or rema";
        LOG("HEDISGenerator", pINFO) << "hadron     = " << hadron     << " -> Pz = " << fPythia->GetP(fPythia->GetN(),3) ;
        LOG("HEDISGenerator", pINFO) << "rema = " << rema << " -> Pz = " << fPythia->GetP(fPythia->GetN()-1,3) ;
